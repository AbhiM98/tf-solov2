...
class SOLOv2Model(tf.keras.Model):
    ...
    def train_step(self, inputs):
        # tf.print("Thisd is input",inputs[3])
        gt_img = inputs[1]
        gt_mask_img = inputs[2]
        gt_boxes = inputs[3]
        gt_cls_ids = inputs[4]
        ...
        # tf.print(tf.shape(gt_img))
        # tf.print(tf.shape(gt_mask_img))
        # tf.print(tf.shape(inputs[3][0]))
        # tf.print(tf.shape(gt_cls_ids))
        # tf.print(tf.shape(gt_labels))
        ...
        with tf.GradientTape() as tape:
            ...
            # tf.print("name of the file",inputs[0])
            # tf.print("name of the file",inputs[0])
...
def evaluate(model, dataset, maxiter, threshold=0.75, **kwargs):
