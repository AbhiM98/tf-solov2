...
# @tf.function
# def correct_bounding(box)
def correct_bounding_box(bboxes):
    x1,y1,x2,y2=bboxes[..., 0],bboxes[..., 1],bboxes[..., 2],bboxes[..., 3]
    swap_indices = tf.squeeze(tf.where(x1>x2),axis=-1)
    squeeze_swap_indices  = swap_indices
    # tf.print(tf.shape(swap_indices))
    x1_swapped = tf.tensor_scatter_nd_min(x1,swap_indices,tf.gather(x2,squeeze_swap_indices))
    x2_swapped = tf.tensor_scatter_nd_min(x2,swap_indices,tf.gather(x1,squeeze_swap_indices))
    swap_indices = tf.where(y1>y2)
    squeeze_swap_indices  = tf.squeeze(swap_indices,axis=-1)
    y1_swapped = tf.tensor_scatter_nd_min(y1,swap_indices,tf.gather(y2,squeeze_swap_indices))
    y2_swapped = tf.tensor_scatter_nd_min(y2,swap_indices,tf.gather(y1,squeeze_swap_indices))
    return tf.stack([x1_swapped,y1_swapped,x2_swapped,y2_swapped],axis=-1)
def compute_solo_cls_targets(inputs,
    ...
    """
    bboxes = inputs[0]
    # tf.print("bbox",bboxes)
    # x1,y1,x2,y2=bboxes[..., 0],bboxes[..., 1],bboxes[..., 2],bboxes[..., 3]
    # swap_indices  = tf.squeeze(tf.where(x1>x2),axis=-1)
    # tf.tensor_scatter_nd_min(x1,swap_indices,tf.gather(x2,swap_indices))
    # tf.print("x",swap_indices)
    # tf.print("wow",tf.shape(swap_indices))
    # tf.print("x2.shape",tf.shape(x1))
    labels = inputs[1]
    classes = inputs[2]
    masks = inputs[3]
    ...
    # bboxes =  correct_bounding_box(bboxes)
    dx = tf.maximum(tf.abs(bboxes[..., 2] - bboxes[..., 0]) * (nx - 1) - 1, 0.)
    dy = tf.maximum(tf.abs(bboxes[..., 3] - bboxes[..., 1]) * (ny - 1) - 1, 0.)
    # tf.print(dx,dy,nx,ny,(bboxes[..., 2] - bboxes[..., 0]),(bboxes[..., 3] - bboxes[..., 1]))
    ...
    # object_scale = tf.abs(object_scale)
