...
def SOLOv2(config):
    ...
    if config.load_pretrained_tf:

        backbone = eval(config.backbone+"(include_top=False,input_shape ="+str(config.imshape)+")")
        x = backbone.layers[-1].output
        x = tf.keras.layers.GlobalAveragePooling2D()(x)
        backbone = tf.keras.Model(inputs = backbone.layers[0].input,outputs = x)
        # tf.print(backbone.summary())
        # exit()
        # backbone = tf.print(backbone,[backbone],"here")
    else:
    ...
    # print(config.connection_layers)
...
def FPN(connection_layers,
    ...
    C1 = connection_layers.get("C1", None)
    ...
    if C1 is not None:
        P2up = layers.UpSampling2D(size=(2, 2),
                                   interpolation=interpolation,
                                   name=name + "FPN_P2upsampled")(P2)
        P2up = layers.Conv2D(pyramid_filters,
                             kernel_size=1,
                             padding='same',
                             use_bias=True,
                             strides=1,
                             kernel_initializer=CONV_INIT,
                             kernel_regularizer=l2(weight_decay),
                             name=name + "FPN_P2upconv_")(P2up)
        C1P1 = layers.Conv2D(pyramid_filters,
                             kernel_size=1,
                             padding='same',
                             use_bias=True,
                             strides=1,
                             kernel_initializer=CONV_INIT,
                             kernel_regularizer=l2(weight_decay),
                             name=name + "FPN_C1P1_")(C1)
        P1 = layers.Add(name=name + "FPN_P1add")([P2up, C1P1])
...
class SOLOv2Model(tf.keras.Model):
    ...
    def call(self, inputs, training=False, **kwargs):
        ...
        # tf.print(tf.shape(gt_mask_img))
        # tf.print(tf.shape(gt_boxes))
        # tf.print(tf.shape(gt_cls_ids))
        # tf.print(tf.shape(gt_labels))
        # inputs=gt_img
        default_kwargs = {
            "score_threshold": 0.5,
            "seg_threshold": 0.5,
    ...
    def train_step(self, inputs):
        ...
        compute_targets_func = partial(util.compute_solo_cls_targets,
                                       shape=(nx, ny),
    ...
    def check_size(self,gt_img,param):
        ...
        def return_same():
            return param
        return tf.cond(tf.rank(gt_img)==3,add_batch_dim,return_same)
...
def train(model,
    ...
    # shape_=[None,]
    model.compute_output_shape((None,model.config.imshape[0],model.config.imshape[1],model.config.imshape[2]))
    if buffer is None:
        buffer = len(train_dataset)
    # for x,y,z,a,b,c in train_dataset:
...
def evaluate_(model, dataset, maxiter, threshold=0.75, **kwargs):
    ...
    for i, data in enumerate(dataset):
        ...
        # gt_img = tf.expand_dims(data[1],0)
        gt_img = data[1]
        gt_mask_img = data[2]
        gt_boxes = data[3]
        gt_cls_ids = data[4]
